...
function findRelevantTools(taskDescription) {
    ...
    if (!taskDescription.trim()) {
        // Show all tools if no search term is provided
        displayAllTools();
        return;
    }
    ...
    // Remove the filter to show all tools
...
// Display all tools function
function displayAllTools() {
    resultsContainer.innerHTML = '';
    aiTools.forEach(tool => {
        displayToolCard(tool, '—'); // Show all without match percentage
    });
...
function displayToolCard(tool, matchPercentage) {
    ...
    const matchDisplay = matchPercentage === '—' ? 
...
// Add "Show All Tools" button to your HTML
// This can be added to the page through HTML or dynamically here
function addShowAllButton() {
    ...
    showAllButton.className = 'btn-primary';
    showAllButton.innerText = 'Show All AI Tools';
    showAllButton.addEventListener('click', displayAllTools);
    
    buttonContainer.appendChild(showAllButton);
    
    // Insert after the search button
    searchBtn.parentNode.insertBefore(buttonContainer, searchBtn.nextSibling);
}
